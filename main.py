...
def register():
    ...
    if form.validate_on_submit():
        ...
        if User.query.filter_by(email=email).first():
            error = "There is already an account registered to that email"
        else:
            new_user = User(
                email=email,
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            error = "There is no user registered to that email"
        else:
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if not current_user.is_authenticated:
            return redirect(url_for("login", error="Log in to leave a comment!"))
        else:
            new_comment = Comment(
